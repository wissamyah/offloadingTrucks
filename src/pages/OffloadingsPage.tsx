...
export const OffloadingsPage = () => {
  ...
  const addTrucks = async (entries: ParsedTruckEntry[]) => {
    ...
    const trucks: Truck[] = entries.map((entry, index) => ({
      id: `truck_${timestamp}_${index}_${Math.random()
        .toString(36)
        .substr(2, 9)}`,
      ...entry,
  ...
  const [offloadModal, setOffloadModal] = useState<{
    open: boolean;
    truck: Truck | null;
  }>({
    open: false,
  ...
  const handleSort = (column: string) => {
    if (sortBy === column) {
      ...
      setSortBy(column);
      setSortDirection(column === "status" ? "desc" : "asc");
    }
  };
    ...
    const sorted = [...filteredTrucks].sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "status": {
          ...
          const aGroup = statusOrder[a.status];
          const bGroup = statusOrder[b.status];

          // If groups differ, use group comparison via generic comparator
          if (aGroup !== bGroup) {
            aValue = aGroup;
            bValue = bGroup;
            break;
          ...
          // Tie-break within the same status by relevant timestamp
          const getRelevantStatusTimestampMs = (t: Truck): number => {
            if (t.status === "scaled_in" || t.status === "offloaded") {
              const match = [...(t.statusHistory || [])]
                .reverse()
                .find((h) => h.status === t.status);
              const ts = match?.timestamp || t.updatedAt || t.createdAt;
              return new Date(ts).getTime();
            }
            return new Date(t.updatedAt || t.createdAt).getTime();
          };

          const aTime = getRelevantStatusTimestampMs(a);
          const bTime = getRelevantStatusTimestampMs(b);
  ...
  const handleConfirmOffload = async (
    netWeight: number,
    deduction?: number
  ) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between gap-2 sm:gap-3">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
              <TruckIcon className="h-6 w-6 sm:h-8 sm:w-8 text-green-500 flex-shrink-0" />
              <div className="min-w-0 flex-1">
                <h1 className="text-lg sm:text-2xl font-bold text-gray-100 truncate">
                  Offloadings Module
                </h1>
                <p className="text-xs sm:text-sm text-gray-400 hidden sm:block">
                  Track and manage paddy truck offloading operations
                </p>
              </div>
            </div>
